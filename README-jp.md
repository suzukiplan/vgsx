...
|0xD200A4 | R41 | M7_EN0 | [Mode 7 有効化: BG0](#0xd200a4-0xd20130-mode-7) |
|0xD200A8 | R42 | M7_EN1 | [Mode 7 有効化: BG1](#0xd200a4-0xd20130-mode-7) |
|0xD200AC | R43 | M7_EN2 | [Mode 7 有効化: BG2](#0xd200a4-0xd20130-mode-7) |
|0xD200B0 | R44 | M7_EN3 | [Mode 7 有効化: BG3](#0xd200a4-0xd20130-mode-7) |
|0xD200B4 | R45 | M7_A0 | [Mode 7 行列 A: BG0](#0xd200a4-0xd20130-mode-7) |
|0xD200B8 | R46 | M7_A1 | [Mode 7 行列 A: BG1](#0xd200a4-0xd20130-mode-7) |
|0xD200BC | R47 | M7_A2 | [Mode 7 行列 A: BG2](#0xd200a4-0xd20130-mode-7) |
|0xD200C0 | R48 | M7_A3 | [Mode 7 行列 A: BG3](#0xd200a4-0xd20130-mode-7) |
|0xD200C4 | R49 | M7_B0 | [Mode 7 行列 B: BG0](#0xd200a4-0xd20130-mode-7) |
|0xD200C8 | R50 | M7_B1 | [Mode 7 行列 B: BG1](#0xd200a4-0xd20130-mode-7) |
|0xD200CC | R51 | M7_B2 | [Mode 7 行列 B: BG2](#0xd200a4-0xd20130-mode-7) |
|0xD200D0 | R52 | M7_B3 | [Mode 7 行列 B: BG3](#0xd200a4-0xd20130-mode-7) |
|0xD200D4 | R53 | M7_C0 | [Mode 7 行列 C: BG0](#0xd200a4-0xd20130-mode-7) |
|0xD200D8 | R54 | M7_C1 | [Mode 7 行列 C: BG1](#0xd200a4-0xd20130-mode-7) |
|0xD200DC | R55 | M7_C2 | [Mode 7 行列 C: BG2](#0xd200a4-0xd20130-mode-7) |
|0xD200E0 | R56 | M7_C3 | [Mode 7 行列 C: BG3](#0xd200a4-0xd20130-mode-7) |
|0xD200E4 | R57 | M7_D0 | [Mode 7 行列 D: BG0](#0xd200a4-0xd20130-mode-7) |
|0xD200E8 | R58 | M7_D1 | [Mode 7 行列 D: BG1](#0xd200a4-0xd20130-mode-7) |
|0xD200EC | R59 | M7_D2 | [Mode 7 行列 D: BG2](#0xd200a4-0xd20130-mode-7) |
|0xD200F0 | R60 | M7_D3 | [Mode 7 行列 D: BG3](#0xd200a4-0xd20130-mode-7) |
|0xD200F4 | R61 | M7_CX0 | [Mode 7 中心 X: BG0](#0xd200a4-0xd20130-mode-7) |
|0xD200F8 | R62 | M7_CX1 | [Mode 7 中心 X: BG1](#0xd200a4-0xd20130-mode-7) |
|0xD200FC | R63 | M7_CX2 | [Mode 7 中心 X: BG2](#0xd200a4-0xd20130-mode-7) |
|0xD20100 | R64 | M7_CX3 | [Mode 7 中心 X: BG3](#0xd200a4-0xd20130-mode-7) |
|0xD20104 | R65 | M7_CY0 | [Mode 7 中心 Y: BG0](#0xd200a4-0xd20130-mode-7) |
|0xD20108 | R66 | M7_CY1 | [Mode 7 中心 Y: BG1](#0xd200a4-0xd20130-mode-7) |
|0xD2010C | R67 | M7_CY2 | [Mode 7 中心 Y: BG2](#0xd200a4-0xd20130-mode-7) |
|0xD20110 | R68 | M7_CY3 | [Mode 7 中心 Y: BG3](#0xd200a4-0xd20130-mode-7) |
|0xD20114 | R69 | M7_TX0 | [Mode 7 移動量 X: BG0](#0xd200a4-0xd20130-mode-7) |
|0xD20118 | R70 | M7_TX1 | [Mode 7 移動量 X: BG1](#0xd200a4-0xd20130-mode-7) |
|0xD2011C | R71 | M7_TX2 | [Mode 7 移動量 X: BG2](#0xd200a4-0xd20130-mode-7) |
|0xD20120 | R72 | M7_TX3 | [Mode 7 移動量 X: BG3](#0xd200a4-0xd20130-mode-7) |
|0xD20124 | R73 | M7_TY0 | [Mode 7 移動量 Y: BG0](#0xd200a4-0xd20130-mode-7) |
|0xD20128 | R74 | M7_TY1 | [Mode 7 移動量 Y: BG1](#0xd200a4-0xd20130-mode-7) |
|0xD2012C | R75 | M7_TY2 | [Mode 7 移動量 Y: BG2](#0xd200a4-0xd20130-mode-7) |
|0xD20130 | R76 | M7_TY3 | [Mode 7 移動量 Y: BG3](#0xd200a4-0xd20130-mode-7) |
|0xD20134 | R77 | M7_DEPTH0 | [Mode 7 遠近投影の角度: BG0](#0xd20134-0xd20140-mode-7-perspective-depth) |
|0xD20138 | R78 | M7_DEPTH1 | [Mode 7 遠近投影の角度: BG1](#0xd20134-0xd20140-mode-7-perspective-depth) |
|0xD2013C | R79 | M7_DEPTH2 | [Mode 7 遠近投影の角度: BG2](#0xd20134-0xd20140-mode-7-perspective-depth) |
|0xD20140 | R80 | M7_DEPTH3 | [Mode 7 遠近投影の角度: BG3](#0xd20134-0xd20140-mode-7-perspective-depth) |
|0xD20144 | R81 | M7_FRAC0 | [Mode 7 移動量の小数部: BG0](#0xd20144-0xd20150-mode-7-fractional-translation) |
|0xD20148 | R82 | M7_FRAC1 | [Mode 7 移動量の小数部: BG1](#0xd20144-0xd20150-mode-7-fractional-translation) |
|0xD2014C | R83 | M7_FRAC2 | [Mode 7 移動量の小数部: BG2](#0xd20144-0xd20150-mode-7-fractional-translation) |
|0xD20150 | R84 | M7_FRAC3 | [Mode 7 移動量の小数部: BG3](#0xd20144-0xd20150-mode-7-fractional-translation) |
|0xD20154 | R85 | M7_FOCAL0 | [Mode 7 焦点距離: BG0](#0xd20154-0xd20160-mode-7-focal-length) |
|0xD20158 | R86 | M7_FOCAL1 | [Mode 7 焦点距離: BG1](#0xd20154-0xd20160-mode-7-focal-length) |
|0xD2015C | R87 | M7_FOCAL2 | [Mode 7 焦点距離: BG2](#0xd20154-0xd20160-mode-7-focal-length) |
|0xD20160 | R88 | M7_FOCAL3 | [Mode 7 焦点距離: BG3](#0xd20154-0xd20160-mode-7-focal-length) |
|0xD20164 | R89 | M7_BACKDROP0 | [Mode 7 矩形外の色: BG0](#0xd20164-0xd20170-mode-7-backdrop) |
|0xD20168 | R90 | M7_BACKDROP1 | [Mode 7 矩形外の色: BG1](#0xd20164-0xd20170-mode-7-backdrop) |
|0xD2016C | R91 | M7_BACKDROP2 | [Mode 7 矩形外の色: BG2](#0xd20164-0xd20170-mode-7-backdrop) |
|0xD20170 | R92 | M7_BACKDROP3 | [Mode 7 矩形外の色: BG3](#0xd20164-0xd20170-mode-7-backdrop) |
...
R0～R40 のアドレスと動作は変更しません。R41～R76 はアフィン変換、R77～R80 は遠近投影の角度、R81～R84 は移動量の小数部、R85～R88 は焦点距離、R89～R92 は元の BG 矩形外の色を設定します。R93～R255 は予約領域です。

Mode 7 は、BG0～BG3 に回転・拡大縮小・スキュー・反転・平行移動のアフィン変換を適用します。4 面すべてに独立した変換を同時に指定できます。既存の `BMPn` で選択する Character Pattern Mode と Bitmap Mode の両方で使用できます。

行列による拡大縮小は [SNES Mode 7](https://wiki.superfamicom.org/mode-7-scaling) を参考にしています。VGS-X では既存の VRAM 形式とレイヤー合成を使用します。元の BG の範囲外は標準では透明ですが、`M7_BACKDROPn` で塗りつぶし色を指定できます。折り返しやタイルの繰り返しは行いません。

#### レジスタの配置と値

BG 番号 `n`（0～3）のアドレスは、表の BG0 アドレスに `4 * n` を加えた値です。アクセスはすべて 4 バイト境界の 32 ビット読み書きです。描画コマンドではなく保持される設定値であり、読み出し時は以下の仕様に従って保存値を返します。

| BG0 アドレス | レジスタ | 形式・意味 | リセット値 |
|:------------:|:---------|:-----------|:----------:|
| 0xD200A4 | M7_EN0-M7_EN3 | ビット 0: 0 = 無効、1 = 有効。ビット 31～1 は書き込みを無視し、読み出しは 0 | 0 |
| 0xD200B4 | M7_A0-M7_A3 | 下位 16 ビットに符号付き 8.8 固定小数点の行列係数 A | 0x00000100 (+1.0) |
| 0xD200C4 | M7_B0-M7_B3 | 下位 16 ビットに符号付き 8.8 固定小数点の行列係数 B | 0 |
| 0xD200D4 | M7_C0-M7_C3 | 下位 16 ビットに符号付き 8.8 固定小数点の行列係数 C | 0 |
| 0xD200E4 | M7_D0-M7_D3 | 下位 16 ビットに符号付き 8.8 固定小数点の行列係数 D | 0x00000100 (+1.0) |
| 0xD200F4 | M7_CX0-M7_CX3 | 中心 X（符号付き 32 ビット整数、ピクセル単位） | 0 |
| 0xD20104 | M7_CY0-M7_CY3 | 中心 Y（符号付き 32 ビット整数、ピクセル単位） | 0 |
| 0xD20114 | M7_TX0-M7_TX3 | 参照元の移動量 X（符号付き 32 ビット整数、ピクセル単位） | 0 |
| 0xD20124 | M7_TY0-M7_TY3 | 参照元の移動量 Y（符号付き 32 ビット整数、ピクセル単位） | 0 |

行列レジスタのビット 31～16 は書き込みを無視し、読み出しは 0 です。下位 16 ビットを 2 の補数の符号付き整数として解釈し、256 で割ります。範囲は -128～127.99609375、刻みは 1/256 です。+0.5 は `0x00000080`、+1.0 は `0x00000100`、-1.0 は `0x0000FF00` です。中心と移動量は 32 ビット全体を 2 の補数で扱い、マスクや範囲制限は行いません。

Mode 7 を無効にしても設定値は保持されます。リセット時は全 BG で無効になり、行列は単位行列、中心と移動量は 0 に戻ります。有効化前に各パラメータを設定し、更新は次の [V-SYNC](#0xe00000in---v-sync) までに完了してください。V-SYNC 時の設定をフレーム全体に使用します。走査線ごとのレジスタや HDMA はありません。遠近投影は [M7_DEPTH](#0xd20134-0xd20140-mode-7-perspective-depth) で指定します。

#### 座標変換

論理画面は 320x200 で、左上が (0, 0)、右が X 正方向、下が Y 正方向です。Mode 7 は既存の 2 倍表示拡大より前に評価します。描画先の各整数座標 `(x, y)` を参照元 BG の座標へ逆変換し、半ピクセルのオフセットは加えません。

`fx`、`fy` は [M7_FRAC](#0xd20144-0xd20150-mode-7-fractional-translation) の符号なし小数部バイト（リセット時は 0）です。`a`、`b`、`c`、`d` は M7_A～M7_D の下位 16 ビットを符号付き整数として解釈した値（256 で割る前）、`cx`、`cy`、`tx`、`ty` は対応する中心と移動量です。
...
座標の減算や最後の加算を含め、中間計算は符号付き 64 ビットで行います。積の和を求めてから丸めます。`floor` は負の無限大方向への切り捨てであり、-1/256 は -1 になります。得られた整数座標の画素だけを参照し、補間やアンチエイリアスは行いません。

行列は描画先から参照元への変換なので、係数 0.5 は表示上の 2 倍拡大です。中心は移動量とスクロールを加える前の、画面と参照元で共通の基点です。正の `tx` / `ty` は参照位置を右 / 下へ移動し、単位行列の場合、表示は左 / 上へ移動します。参照元の点 `(px, py)` を画面の `(cx, cy)` に置くには、`tx = px - cx - scroll_x`、`ty = py - cy - scroll_y` とします。

特異行列も有効です。係数がすべて 0 の場合、全描画先画素が同じ点 `(cx + tx + scroll_x, cy + ty + scroll_y)` を参照します。係数 0 は BG の無効化や非表示を意味しません。

#### 参照元の範囲と合成

| モード | 有効な参照元座標 | 画素の参照と透明色 |
|:-------|:-----------------|:-------------------|
| Character Pattern | `0 <= source_x < 2048`, `0 <= source_y < 2048` | 256x256 のネームテーブルから 8x8 タイルと画素を選び、上下左右反転とパレット属性を適用。色インデックス 0 は透明 |
| Bitmap | `0 <= source_x < 320`, `0 <= source_y < 200` | ネームテーブル内の RGB888 画素を参照。値 `0x00000000` は透明 |

「範囲外」は表示中の 320x200 領域ではなく、参照元 BG 全体の範囲外を意味します。VRAM にアクセスする前に座標を検査し、座標のマスクや折り返しは行いません。`M7_BACKDROPn = 0` のとき、範囲外では合成済みの画素を変更せず、下位 BG・スプライト・通常の画面背景が見えます。

BG の順序、`SPOS`、`SKIP`、`SKIP0`～`SKIP3` の動作は変わりません。ビットマップの描画・クリア・転送・画素読み出しは変換前の VRAM に対して行います。Bitmap Mode のウィンドウは描画先画面のクリッピング矩形であり、変換しません。

Character Pattern Mode では行列変換後に既存のスクロール原点を加えます。単位行列・移動量 0 なら範囲内では通常のスクロールと一致し、境界を越えると折り返さず `M7_BACKDROPn`（標準は透明）を使用します。Bitmap Mode の `SXn` / `SYn` 書き込みは従来どおり VRAM を即座にスクロールして空いた部分をクリアするため、その値を Mode 7 描画で再加算しません。VRAM を変更しない移動には `M7_TXn` / `M7_TYn` を使用します。

`M7_ENn = 0` の場合、すべての Mode 7 パラメータを無視し、Character Pattern Mode の折り返しを含め従来の描画を行います。

#### 設定例

対象 BG の `M7_ENn = 1`、`M7_CXn = 160`、`M7_CYn = 100`、`M7_TXn = M7_TYn = 0` とします。Character Pattern Mode では `SXn = SYn = 0` にも設定します。表の係数は 32 ビットのレジスタ値です。

| (160, 100) を中心とする表示効果 | M7_An | M7_Bn | M7_Cn | M7_Dn |
|:-------------------------------|:-----:|:-----:|:-----:|:-----:|
| 等倍 | 0x00000100 | 0 | 0 | 0x00000100 |
| 2 倍拡大 | 0x00000080 | 0 | 0 | 0x00000080 |
| 1/2 に縮小 | 0x00000200 | 0 | 0 | 0x00000200 |
| 時計回りに 90 度回転 | 0 | 0x00000100 | 0x0000FF00 | 0 |
| 水平スキュー（表示 X のずれ = 0.5 * (Y - 100)） | 0x00000100 | 0x0000FF80 | 0 | 0x00000100 |

表示を時計回りに `theta` 回転し、正の倍率 `s` で均等に拡大縮小する場合、`A = cos(theta)/s`、`B = sin(theta)/s`、`C = -sin(theta)/s`、`D = cos(theta)/s` を符号付き 8.8 に量子化します。係数の範囲内に収めてください。単一のアフィン行列によるスキューは BG 全体で一定です。走査線ごとに倍率が変わる遠近感には M7_DEPTH を使用します。

#### 検証項目

- 4 面で異なる変換と両モードを混在させ、レイヤー順序とスプライト挿入位置を確認する。
- 等倍・拡大・縮小・回転・スキュー・負の係数・特異行列を座標式と照合する。
- 参照元座標 -1、0、幅/高さ - 1、幅/高さ、小数を含む負の値を確認する。範囲外は折り返さず、余白色 0 なら下位レイヤーを表示する。
- タイル反転・パレット・透明画素・ビットマップのクリッピング・両モードのスクロールを確認する。
- レジスタ読み戻し・無視されるビット・リセット・再有効化・フレーム単位の設定反映・極端な中心/移動量でのオーバーフローや範囲外参照を確認する。

### 0xD20134-0xD20140: Mode 7 Perspective Depth

`M7_DEPTHn` は BG `n` のアフィン変換後の平面に遠近投影を加えます。4 バイト境界の 32 ビット符号なし整数で角度を指定し、書き込み時に 0～75 度へ制限されます。読み出しは制限後の値です。リセット値は全 BG で 0 です。0 は従来のアフィン描画をそのまま維持し、Mode 7 無効時は無視されます。

論理画面 320x200 を投影の基準平面とします。角度を大きくすると奥（上辺）が手前（下辺）より狭くなり、高さも圧縮されます。下辺は画面幅に合わせて論理行 199 に配置され、上下方向の余白は上側に集まり透明になります。基準矩形の投影は台形ですが、その斜辺で描画を切りません。水平方向は基準平面を越えて逆変換し、参照元 BG が有効な範囲を描画します。元の BG の境界・透明色・タイル属性・レイヤー順序・ビットマップのクリッピングは引き続き適用されます。

焦点距離は `F = M7_FOCALn`（初期値 200 論理ピクセル）です。VGS-X の整数角度の三角関数テーブルを使い、`s = vgsx_sin[depth] / 256`、`c = vgsx_cos[depth] / 256` とすると、平面座標 `(X, Y)` の投影は次のとおりです。
...
描画時は、各描画先座標 `(x, y)` に対して次の逆投影を行ってから、アフィン行列を適用します。
...
`denom <= 0`、`Y < 0`、`Y >= 200` は透明とします。`X` は 0～319 に制限せず、最終的な参照元座標で BG の範囲を検査します。行 199 では下辺を正確に合わせるため `v = 99` とします。丸め前の `(X, Y)` をアフィン座標式の `(x, y)` に代入し、最後の参照元座標のみを切り捨てます。逆投影は倍精度浮動小数点で計算します。中間フレームバッファを再サンプリングする処理ではありません。

中心 `(M7_CXn, M7_CYn)` は**投影前**の平面座標です。その投影後の画面位置はスクロールや回転中も固定されます。表示平面の上下中央に追跡点を置く場合、`CX = 160`、`CY` は `100 + (19800 * S - F * 256) / (F * 512 + S)` に最も近い整数とします（`S = vgsx_sin[depth]`）。参照元の追跡点は通常どおり TX/TY でこの中心へ合わせます。

### 0xD20144-0xD20150: Mode 7 Fractional Translation

`M7_FRACn` は参照元の移動量の小数部です。ビット 7～0 が X、15～8 が Y で、それぞれ符号なし 0～255（0～255/256 ピクセル）です。ビット 31～16 は書き込みを無視し、読み出しは 0 です。4 バイト境界の 32 ビットアクセスを使用します。リセット値は全 BG で 0 です。

アフィン描画・遠近投影とも、最終的な参照元座標の `floor` **より前**に加算します。Mode 7 無効時は無視され、VRAM は変更しません。整数移動量 TX/TY の意味は変わりません。負の移動量は切り捨てた整数部と正の小数部で表します。例えば -0.5px は `TX = -1`、X 小数部 128 です。

正の Q8.8 追跡座標では、整数部からカメラ中心を引いた値を TX/TY に、下位 8 ビットを FRAC に設定します。全書き込みを V-SYNC 前に完了してください。X と Y を個別に整数へ丸めた際の、斜め移動時の横ぶれを防ぎます。色の補間は行わず、サブピクセル位置を保持する機能です。

### 0xD20154-0xD20160: Mode 7 Focal Length

`M7_FOCALn` は BG `n` の焦点距離（論理ピクセル）です。4 バイト境界の 32 ビット符号なし整数で、書き込み時に 100～4096 へ制限され、読み出しは制限後の値です。リセット値は全 BG で 200 です。M7_DEPTH が 0、または Mode 7 無効時は無視されます。

短い焦点距離ほど手前と奥の倍率差が大きくなります。下辺は画面幅に合わせたまま行 199 に固定されます。長い焦点距離では平行投影に近づきます。カメラ基点の投影位置を計算するときも同じ `F` を使用してください。下限値は、対応する全角度で基準平面内の投影分母を正に保つためのものです。

### 0xD20164-0xD20170: Mode 7 Backdrop

`M7_BACKDROP0`～`M7_BACKDROP3` は、変換後の参照位置が元の BG 矩形（Character Pattern Mode は 2048x2048、Bitmap Mode は 320x200）の外に出た場合の RGB888 色です。4 バイト境界の 32 ビットレジスタでビット 23～0 を使用し、上位ビットは書き込みを無視して読み出しは 0 になります。リセット値は全 BG で 0（透明）です。非 0 なら、回転・拡大縮小・移動・遠近投影の参照によって生じる矩形外の余白を、その BG の通常のレイヤー位置で塗りつぶします。

元の矩形内の透明画素は透明のままです。遠近投影で生じる平面上側の余白も透明を維持し、別 BG の空などを表示できます。ビットマップのウィンドウと BG 描画スキップは引き続き適用されます。Mode 7 無効時は無視されます。
...
| cg:bg+mode7 | `vgs_mode7_enable` | `(bg, ON/OFF)` で Mode 7 を有効・無効にする |
| cg:bg+mode7 | `vgs_mode7_abcd` | `(bg, a, b, c, d)` で逆アフィン行列を指定する。係数は符号付き 8.8（256 = 1.0） |
| cg:bg+mode7 | `vgs_mode7_camera` | `(bg, cx, cy)` で変換中心を指定する。符号付き整数の論理ピクセル座標 |
| cg:bg+mode7 | `vgs_mode7_translate` | `(bg, tx, ty)` で参照元の移動量を指定する。符号付き整数のピクセル単位 |
| cg:bg+mode7 | `vgs_mode7_frac` | `(bg, x, y)` で移動量の小数部を指定する。各軸 0～255、1/256px 単位 |
| cg:bg+mode7 | `vgs_mode7_depth` | `(bg, degrees)` で遠近投影の角度を指定する。符号なし整数、0～75 度に制限 |
| cg:bg+mode7 | `vgs_mode7_focal` | `(bg, pixels)` で焦点距離を指定する。符号なし整数の論理ピクセル、100～4096 に制限 |
| cg:bg+mode7 | `vgs_mode7_backdrop` | `(bg, color)` で元の BG 矩形外の色を指定する。RGB888、0 は透明 |
