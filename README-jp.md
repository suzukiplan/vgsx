...
両者は、「固定された仮想ハードウェアモデルを定義し、それをエミュレーションによって長期にわたり維持する」という共通の思想を持ち、実在のハードウェアや OS から独立した環境を提供します。

VGS-Zero は、8bit Z80 ベースの計算機モデルを前提とし、強い制約の中での開発とレトロスタイルの思考モデルを重視して設計されました。
VGS-X はこのモデルを 32bit MC68030 ベースのアーキテクチャとして再定義し、開発のしやすさを維持したまま、計算量・メモリ容量・資産規模の上限を拡張しています。

VGS-X は、性能や新規性を競うためのシステムではありません。
開発者が思考モデルを変えることなく、長期にわたってゲームを作り続けられる、
安定した仮想ゲームプラットフォームを提供することを目的としています。
